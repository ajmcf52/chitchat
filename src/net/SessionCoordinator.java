...
public class SessionCoordinator extends Worker {
    ...
    /**
     * used to compare the participant count received from the Registry
     * with what we have.
     * The SM content format is as follows: "OK; <participantCount> users now chatting."
    ...
    public void run() {
        ...
        /**
         * SessionCoordinator's main responsibility is to wait for & handle
         * Join and Exit messages.
         */
        while (true) {
            Socket socket = null;
            Object obj = null;
            ...
            try {
                /**
                 * new connection incoming; let's handle it &
                 * see what they want.
                 */
            ...
            if (msg instanceof JoinRoomMessage) {
                /**
                 * in the case of a JoinRoom request, we simply initialize the user.
                 * everything is taken care of in that function.
                 */
     ...
     * @param initialMessage first message to be sent to all users in the room. Hosts receive a special version.
     ...
     */
    public void initializeUser(String alias, Socket socket, boolean isHosting, ObjectInputStream in, ObjectOutputStream out) {

        if (!isHosting) {
            /**
             * if isHosting is false, this message is sent to a JoinRoomWorker;
             * all they expect in return is a SimpleMessage with the text "OK".
             * (ChatUser's WelcomeMessage is handled and sent later in the function)
             */
        ...
        /**
         * in any case, we need to initialize some field variables to open 
         * up some communications pathways, both for hosts and non-hosts alike.
         */
        ...
        if (isHosting) {
            // in this case, we queue up the sending of a single host-centric WelcomeMessage.
             ...
             * and simply pretend we receive the message, when really, as Coordinator, we are
             * interjecting and passing a special request into the incoming queue for a
            ...
            /**
             * Enter here if isHosting is false.
